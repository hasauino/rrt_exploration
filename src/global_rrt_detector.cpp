...
void rvizCallBack(const geometry_msgs::PointStamped::ConstPtr& msg)
{
  ...
  nav_msgs::MapMetaData info = mapData.info;
  tf::poseMsgToTF(info.origin, transform);
  // Transform the clicked point into map grid cell
  tf::Point transPt = transform.inverse() * pt / info.resolution;
  int x = ceil(transPt.getX()) - 1;
  int y = ceil(transPt.getY()) - 1;
  if (x > 0 && x < info.width && y > 0 && y < info.height)
    ROS_INFO("Point-%d(%d, %d): %d", (int)points.points.size(), x, y, mapData.data[info.width * y + x]);
  else
    ROS_INFO("Point-%d(%d, %d): Outside Range", (int)points.points.size(), x, y);
}
...
int main(int argc, char** argv)
{
  ...
  p.y = 0;
  points.points.push_back(p);
  std::cout << "Global RRT Initialized" << std::endl;
